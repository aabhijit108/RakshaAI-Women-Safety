...
TaskManager.defineTask(LOCATION_TRACKING_TASK, async ({ data, error }: any) => {
  ...
  if (data) {
    try {
      ...
      const isMonitoringActive = await AsyncStorage.getItem("is_monitoring_active");
...
const sendBackgroundUpdate = async (latitude: number, longitude: number) => {
  try {
    ...
    const activeContacts = JSON.parse(savedData).filter((c: any) => c.isOn === true);
...
export default function HomeScreen() {
  ...
  try {
    ...
    const micRes = await SpeechRecognitionModule.ExpoSpeechRecognitionModule.requestPermissionsAsync();
  ...
  const handleSpecificShare = async (type: 'EMAIL' | 'WHATSAPP' | 'SMS') => {
    try {
      ...
      const savedData = await AsyncStorage.getItem("emergency_contacts");
      if (!savedData) return;
      const contacts = JSON.parse(savedData).filter((c: any) => c.isOn === true);
      if (contacts.length === 0) {
        Alert.alert("No Contacts", "Please add active emergency contacts first.");
        return;
      }
        ...
        for (const contact of contacts) {
          if (contact.email) {
            await fetch("http://localhost:5000/send-location-email", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ latitude, longitude, recipientEmail: contact.email }),
            });
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.actionGrid}>
          ...
          <TouchableOpacity style={styles.actionCard} onPress={() => setShareModalVisible(true)}>
            <View style={styles.greenIconBg}><MapPin size={22} color="#00C4B4" /></View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.shareSheet}>
            ...
            <View style={styles.shareOptions}>
              <TouchableOpacity style={styles.shareItem} onPress={() => handleSpecificShare('EMAIL')}>
                <View style={[styles.shareIconBg, { backgroundColor: '#1e2d3d' }]}><Mail color="#556ee6" size={24} /></View>
                <Text style={styles.shareText}>Email</Text>
              </TouchableOpacity>

              <TouchableOpacity style={styles.shareItem} onPress={() => handleSpecificShare('WHATSAPP')}>
                <View style={[styles.shareIconBg, { backgroundColor: '#0a2e2a' }]}><MessageCircle color="#25D366" size={24} /></View>
                <Text style={styles.shareText}>WhatsApp</Text>
              </TouchableOpacity>

              <TouchableOpacity style={styles.shareItem} onPress={() => handleSpecificShare('SMS')}>
                <View style={[styles.shareIconBg, { backgroundColor: '#2a1a1a' }]}><MessageSquare color="#e63946" size={24} /></View>
...
const styles = StyleSheet.create({
  ...
  shareSheet: { backgroundColor: '#0f1a2a', borderTopLeftRadius: 25, borderTopRightRadius: 25, padding: 25, paddingBottom: 65 },
